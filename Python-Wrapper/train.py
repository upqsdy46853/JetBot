...
def reset():
    global robot, action, frames, states, next_states, rewards, acc_rewards, cnt
    robot.reset()
    action = 0
    frames = 0
    states = []
...
def execute(change):
    ...
    if cnt > 10 or change['done']: 
        ...
        if best_mean < mean_reward:
            ...
            torch.save(net.state_dict(), "best_model.dat", _use_new_zipfile_serialization=False)
